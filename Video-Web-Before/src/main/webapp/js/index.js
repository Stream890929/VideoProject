...
$('nav .nav_down').mouseenter(function () {
    ...
    $(this).children('img').attr('src', 'http://localhost:8080/VideoProject/img/nav_down2.png');
});
$('nav .nav_down').mouseleave(function () {
    $('#nav_down').stop().slideUp();
    $(this).children('img').attr('src', 'http://localhost:8080/VideoProject/img/nav_down.png');
});
...
function commitRegForm() {
    ...
    if (regIsCommitEmail && regIsCommitPsw && verifyCode.validate(code)) {
        //用js提交表单
        $.ajax({
            url: "/VideoProject/user/insertUser",
...
function commitLogin() {
    ...
    if (null != email && email != "" && null != password && password != "") {
        ...
        $.post("/VideoProject/user/loginUser", params, function (data) {
            if (data == 'success') {
                //登录框消失
                $("#login").addClass("hidden");
